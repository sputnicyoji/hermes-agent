...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # Identity override: append as the LAST system message, closest to
        # model output. Counteracts upstream proxy-injected identity prompts
        # (OneHub / One-API presets). Placed before cache_control so it
        # participates in the cache breakpoint and stays byte-stable across
        # turns (config-driven value, doesn't change mid-session).
        if getattr(agent, "_identity_override", None):
            api_messages.append({"role": "system", "content": agent._identity_override})
