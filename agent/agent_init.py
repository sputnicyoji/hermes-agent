...
def init_agent(
    ...
    # Identity override: counteract upstream OpenAI-compatible proxies (OneHub /
    # One-API presets) that silently inject their own 'you are X' system prompt.
    # Configured via ``model.identity_override`` in config.yaml; appended as the
    # LAST system message in conversation_loop so it sits closest to model output.
    try:
        from hermes_cli.config import load_config as _lc
        agent._identity_override = (cfg_get(_lc(), "model", "identity_override", default="") or "").strip() or None
    except Exception:
